...
export default function DataView() {
  ...
  const handleLimitChange = (newLimit) => {
    localStorage.setItem("limit", newLimit.toString()); // lưu vào ls
    dispatch(setLimit(newLimit)); // cập nhật state
    dispatch(loadUsers({ page: 1, limit: newLimit, sortBy, order })); // gọi load lại page 1
  };
  ...
  const handleSortChange = (newSortBy, newOrder) => {
    ...
    dispatch(setSort({ sortBy: newSortBy, order: newOrder }));  // cập nhật state
    dispatch(loadUsers({ page: 1, limit: limit, sortBy: newSortBy, order: newOrder })); // gọi load lại page 1
  };
