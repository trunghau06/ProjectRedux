...
export default function DataView() {
  ...
  // Load trang 1 lần đầu (chỉ sau khi đã khôi phục settings)
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* Pagination cho mobile - hiển thị trước cards */}
      {isMobile && !isOpen && data.length > 0 && (
        <Pagination
      ...
      {/* Pagination cho desktop - hiển thị sau bảng */}
      {!isMobile && !isOpen && data.length > 0 && (
        <Pagination
          currentPage={paginationPage}
          totalItems={data.length}
      ...
      {/* Debug info */}
      {isMobile && (
        <div style={{ position: 'fixed', top: 10, right: 10, background: 'yellow', padding: 5, zIndex: 9999, fontSize: 12 }}>
          Mobile: {isMobile ? 'YES' : 'NO'}<br/>
          isOpen: {isOpen ? 'YES' : 'NO'}<br/>
          data: {data.length}<br/>
          Render Pagination: {(!isOpen && data.length > 0) ? 'YES' : 'NO'}
        </div>
      )}
    </>
  );
