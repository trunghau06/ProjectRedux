...
export default function PaginationWrapper() {
  ...
  const handlePageChange = useCallback(async (newPage) => {
    ...
    setTimeout(() => {
      ...
      // Kiểm tra xem đang ở table hay card view
      ...
      if (isMobile) {
        ...
        if (cards?.[targetIndex]) {
          // Scroll container đến vị trí card
          const cardTop = cards[targetIndex].offsetTop;
          container.scrollTo({ top: cardTop - 100, behavior: "smooth" }); // -100 để có khoảng trống phía trên
        }
      } else {
