...
export default function DataView() {
  ...
  // Theo dõi màn hình
  useEffect(() => {
    const handleResize = () => setIsMobile(window.innerWidth <= 768); // mỗi lần cửa số thay đổi
    window.addEventListener("resize", handleResize);
    return () => window.removeEventListener("resize", handleResize);  // xóa sự kiện khi unmount
  }, []);

  // Hàm xử lí khi scroll
  const handleScroll = useCallback(() => {
    const container = containerRef.current;   // Lấy container cuộn
    if (!container || loading || isLoadingRef.current || !hasMore) return;  

    const { scrollTop, scrollHeight, clientHeight } = container;  // Lấy thông tin scroll
    if (scrollHeight - scrollTop - clientHeight <= 5) { // Đến gần cuối
