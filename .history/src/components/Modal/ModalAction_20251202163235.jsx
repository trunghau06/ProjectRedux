...
export default function AddRecordModal() {
  ...
  const { isOpen, isEditMode, editingUser } = useSelector(state => state.modal);  // useState modal
  ...
  useEffect(() => {
    if (isOpen) {
      if (isEditMode && editingUser) {  // nếu là edit -> điền sẵn dữ liệu
        setFormData({
        ...
        resetForm();  // nếu là add mới -> reset form
      }
    }
  ...
  const handleInputChange = (e) => {  // Cập nhật dữ liệu form -> Nhập thì cập nhật vào form
    const { name, value } = e.target;
    setFormData(prev => ({ ...prev, [name]: value }));
  };

  const handleAvatarChange = (e) => { // Xử lí ảnh avatar
  ...
  const resizeImage = (file, maxWidth = 80, maxHeight = 80) => {  // Resize ảnh
    return new Promise((resolve, reject) => { // Promise để xử lí bất đồng bộ
      const reader = new FileReader(); // đọc file
      
      reader.onload = (event) => {  // xử lí khi đọc xong
        const img = new Image();
        img.onload = () => {
  ...
  const handleBackdropClick = (e) => {  // click bên ngoài đóng modal
          ...
          <div className="form-grid">
            ...
            <div className="form-group">
              ...
              <input
                ...
                placeholder="Nhập tên (tối thiểu 10 ký tự)"
                required
                minLength="10"
              />
            </div>
