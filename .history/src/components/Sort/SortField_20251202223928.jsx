...
export default function SortableHeader({ label, sortKey }) {
    ...
    const handleSortClick = () => {
        ...
        // 1. Cập nhật state Redux (setSort sẽ reset page=1, data=[])
        dispatch(setSort({ sortBy: sortKey, order: newOrder }));
        
        // 2. Tải lại dữ liệu trang 1 với cấu hình mới
        dispatch(loadUsers({ 
            page: 1, 
            limit: currentLimit, 
            sortBy: sortKey, 
...
// Lưu ý: Để hiển thị đẹp, bạn cần thêm CSS cho các class .sortable-header, .sort-icon vào file DataTable.css