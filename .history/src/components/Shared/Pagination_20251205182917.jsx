...
export default function Pagination({
  ...
  const current = Math.min(currentPage, maxPage); // giới hạn page không vượt quá maxPage

  const startRecord = totalItems === 0 ? 0 : (current - 1) * itemsPerPage + 1;
  const endRecord   = Math.min(current * itemsPerPage, totalItems); // không vượt quá totalItems
  ...
  return (
    <div className="pagination-container">
      ...
      <button
        className="pagination-btn"
        onClick={handleNext}
        disabled={current >= maxPage || loading}
      >
        Next →
      </button>
    </div>
