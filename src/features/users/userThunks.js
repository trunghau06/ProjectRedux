...
export const addUser = createAsyncThunk(  // thêm dữ liệu
  "users/addUser",
  async (userData) => {  // Nhận object mới
    const response = await fetch(API_URL, {   // gửi dữ liệu lên server
      method: "POST",
    ...
    if (!response.ok) {
      ...
      throw new Error(`Thêm dữ liệu thất bại: ${response.status}`);
    }
...
export const editUser = createAsyncThunk( // chỉnh sửa dữ liệu
  "users/editUser",
  async ({ id, updates }) => {  // id -> user muốn sửa , updates -> chứa các field sửa
    // Gửi PUT 
...
export const deleteUser = createAsyncThunk( // xóa dữ liệu
  "users/deleteUser",
  async (userId) => {
